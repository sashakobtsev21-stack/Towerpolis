namespace Towerpolis.Core.Meta
{
    ...
    /// for the gameplay track (Magnet) so the shared seed stays fair across devices.
    /// </summary>
    public readonly struct UpgradeState
    {
        ...
        public UpgradeState(int magnetLevel, int cityBonusLevel)
        {
        ...
        public static UpgradeState Default => new UpgradeState(0, 0);

        // Copy-with helpers so the caller can apply a single purchase result without rebuilding by hand.
        public UpgradeState WithMagnet(int level) => new UpgradeState(level, CityBonusLevel);
        public UpgradeState WithCityBonus(int level) => new UpgradeState(MagnetLevel, level);
    }
}
